...
<body>
  ...
  <div class="navbar navbar-inverse navbar-fixed-top"><!--***HEADER****---->
      <div class="navbar-inner">
        ...
        $select_sql = "SELECT  * FROM consultation WHERE daterenouvelle <= '$inc_date' and statut='ACCORD' ";
...
<!-- For more projects: Visit codeastro.com  -->
      <li><a href="../home.php?invoice_number=<?php echo $_GET['invoice_number']?>"><span class="icon-th"></span>Accueil</a></li>
     ...
     <div class="container"><!---****SEARCHES_CONTENT*****--->
      ...
      <div class="row">
		...
		<div class="post-entry" align="center">
            ...
            <input type="text"  id="name_med1" size="4"  onkeyup="med_name1()" placeholder="Filtre par numero" title="Numero consultation">
            <input type="text" size="4"  id="med_quantity" onkeyup="quanti()" placeholder="Filtre par nom" title="departement">
            <input type="text" size="4" id="med_exp_date" onkeyup="exp_date()" placeholder="Filtre par Adresse" title="renouvellement">
			            <input type="text" size="4" id="cons_exp_date" onkeyup="cons_date()" placeholder="Par Session" title="consultation">
						            <input type="text" size="4" id="cons_fin_date" onkeyup="fin_date()" placeholder="Par Filiere" title="consultation">


					    <input type="text" size="4" id="service_status" onkeyup="service_search()" placeholder="Filtre par statut" title="service">
			            


           <a href="index.php?invoice_number=<?php echo $_GET['invoice_number']?>" id="popup"><button class="btn btn-success btn-md" name="submit"><span class="icon-plus-sign icon-large"></span> Ajouter Inscription</button></a>
  ...
  <!-- For more projects: Visit codeastro.com  -->
    ...
    <?php
       ...
       include('../dbcon.php');
         ...
         $select_sql = "SELECT * from utilisateur where code_profil='Client'";
    ...
    ?>
      ...
      <div style="text-align:center;">
        Total De Inscrits : <font color="green" style="font:bold 22px 'Aleo';">[<?php echo $row;?>]</font>
      </div>
    ...
    <div class="row">
      <div class="col-12">
        <form method="POST">
          ...
          <table id="table0" class="table table-bordered table-striped table-hover">
           <thead>
             ...
             <th width="1%">date Naissance</th>
             <th  width="1%">Email</th>
             <th width="1%">Session</th>
             <th width="1%">Filiere</th>
             <th width = "3%" style="background-color: #c53f3f;" width="1%">Statut</th>
        ...
        <?php $sql = "SELECT * FROM utilisateur where  code_profil='Client'"; ?>
      ...
      <!--Use a while loop to make a table row for every DB row-->
        ...
        <tr style="">
            ...
            <td><img height="125" width="100" src="data:image;base64,<?php echo $row['photo']?>"></td>
            ...
            ?></td>
            <td><a id="popup" href="update_view.php?id=<?php echo $row['code_user']?>&invoice_number=<?php echo $_GET['invoice_number']?>"><button class="btn btn-info"><span class="icon-edit"></span></button></a>
          <button class="btn btn-danger delete" id="<?php echo $row['code_user']?>"><span class="icon-trash"></span>&nbsp;</button></td>
...
function quanti() {//***Search For departement *****
  var input, filter, table, tr, td, i;
...
function cons_date() {//***Search For expireDate *****
  var input, filter, table, tr, td, i;
  input = document.getElementById("cons_exp_date");
  filter = input.value.toUpperCase();
  table = document.getElementById("table0");
  tr = table.getElementsByTagName("tr");
  for (i = 0; i < tr.length; i++) {
...
function fin_date() {//***Search For expireDate *****
  var input, filter, table, tr, td, i;
  input = document.getElementById("cons_fin_date");
  filter = input.value.toUpperCase();
  table = document.getElementById("table0");
  tr = table.getElementsByTagName("tr");
  for (i = 0; i < tr.length; i++) {
...
function age_search() {//***Search For Status*****
  var input, filter, table, tr, td, i;
  input = document.getElementById("age_status");
  filter = input.value.toUpperCase();
  table = document.getElementById("table0");
  tr = table.getElementsByTagName("tr");
  for (i = 0; i < tr.length; i++) {
...
function service_search() {//***Search For Status*****
  var input, filter, table, tr, td, i;
  input = document.getElementById("service_status");
  filter = input.value.toUpperCase();
  table = document.getElementById("table0");
  tr = table.getElementsByTagName("tr");
  for (i = 0; i < tr.length; i++) {
