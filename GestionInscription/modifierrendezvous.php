  ...
  <body>
    ...
    <section class="site-section">
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <h2 class="mb-5 text-center">RENDEZ-VOUS ACCORDE!</h2>
...
//DATE DE NAISSANCE
      $annee=htmlspecialchars($_POST['daterend']);
      $date1=strtotime($annee);
      $date2=date('Y-m-d',$date1);
	  $statut="Accord";

      $heure=htmlspecialchars($_POST['heure']);
      $idp=htmlspecialchars($_POST['idclientp']);
	  $idm=htmlspecialchars($_POST['idclientm']);
	  $idd=htmlspecialchars($_POST['idclientd']);
...
//$message="Modification effectuée avec succes";
     ...
     // if (!empty($nom) AND !empty($prenom) AND !empty($date2) AND !empty($sexe) AND !empty($telephone) AND !empty($mdp)) {
        $sql="UPDATE rendez_vous set date_rendezvous='$date2',heure='$heure',statut='$statut' where idpatients='$idp' and idmedecin='$idm' and date='$idd'";
        if (mysqli_query($conn,$sql)){
          $message="Accord effectuée avec succes";
		  echo "'<script type='text/javascript'>alect('$message')</script>";
        }else{
